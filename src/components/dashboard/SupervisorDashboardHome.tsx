...
type SupervisorDashboardHomeProps = {
  userName: string;
    ...
    <div className="space-y-6 py-4 animate-fade-in md:space-y-6 md:py-6">
      <header className="space-y-1.5">
        ...
        <p className="text-base text-slate-600 md:text-base md:text-slate-500">
          {roleLabel('supervisor')} • {dateLabel}
        </p>
        <p className="hidden text-sm text-slate-500 md:block">{t('roleHints.staff')}</p>
      </header>

      <div className="grid grid-cols-2 gap-3 md:hidden">
        <MobileCompactStat title={t('stats.attendance')} value={attendanceDisplay} tone="emerald" />
        <MobileCompactStat
      ...
      <div className="md:hidden">
        <MobilePrimaryCta
          label={t('supervisorMobile.primaryCta')}
          icon={<AlertCircle aria-hidden />}
          onClick={() => navigate('/absences')}
        />
        <p className="sr-only">{t('supervisorMobile.primaryCtaHint')}</p>
      </div>
